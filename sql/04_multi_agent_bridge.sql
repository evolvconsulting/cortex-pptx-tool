...
CREATE OR REPLACE NETWORK RULE {{DATABASE}}.{{SCHEMA}}.PPTX_AGENT_EGRESS_RULE
    MODE = EGRESS
    TYPE = HOST_PORT
...
CREATE OR REPLACE SECRET {{DATABASE}}.{{SCHEMA}}.PPTX_KEYPAIR_SECRET
    TYPE = GENERIC_STRING
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION PPTX_AGENT_EXTERNAL_ACCESS
    ALLOWED_NETWORK_RULES = ({{DATABASE}}.{{SCHEMA}}.PPTX_AGENT_EGRESS_RULE)
...
CREATE OR REPLACE PROCEDURE {{DATABASE}}.{{SCHEMA}}.CALL_PPTX_AGENT_EXTERNAL(PRESENTATION_REQUEST VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
...
DECLARE
    result STRING;
BEGIN
    SELECT {{DATABASE}}.{{SCHEMA}}.ASK_PPTX_AGENT(:PRESENTATION_REQUEST) INTO :result;
    RETURN result;
END;
$$;
